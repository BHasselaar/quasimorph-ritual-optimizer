...
from .confirmed_sprites import USER_CONFIRMED_SPRITE_ALIASES
...
from .save_sync import SaveSnapshot, read_save
...
EXPECTED_COMPONENT_SPRITE_COUNT = len(USER_CONFIRMED_SPRITE_ALIASES)


COMMON_GAME_PATH_HINTS = (
    r"C:\Program Files (x86)\Steam\steamapps\common\Quasimorph",
    r"D:\SteamLibrary\steamapps\common\Quasimorph",
)
...
def _component_sprite_matches_record(item: Item) -> bool:
    if item.internal_id not in USER_CONFIRMED_SPRITE_ALIASES:
        return False
    if _component_sprite_missing(item):
        return False
    expected = sprite_cache_dir() / f"{item.internal_id}.png"
    try:
        return Path(item.sprite_path).resolve() == expected.resolve()
    except OSError:
        return Path(item.sprite_path).name == expected.name


def _confirmed_sprite_count(items: list[Item]) -> int:
    return sum(1 for item in items if _component_sprite_matches_record(item))


def _all_confirmed_sprites_ready(items: list[Item]) -> bool:
    return _confirmed_sprite_count(items) >= EXPECTED_COMPONENT_SPRITE_COUNT


def _items_with_save_snapshot(
    items: list[Item],
    ...
    for item in items:
        if not item.internal_id:
            updated.append(item)
            continue
        quantity = int(snapshot.quantities.get(item.internal_id, 0))
        enabled = item.enabled
        if quantity <= 0:
            enabled = False
        elif item.quantity <= 0:
            enabled = True
        updated.append(replace(item, quantity=quantity, enabled=enabled))
    return updated
...
class RitualOptimizerApp(tk.Tk):
    def __init__(self):
        ...
        self._build_ui(); self._refresh_inventory(); self._refresh_setup_controls(); self.bind("<FocusIn>",lambda _event:self._refresh_setup_controls()); self.after(100,self._poll_worker); self.protocol("WM_DELETE_WINDOW",self._on_close)
    ...
    def _build_ui(self):
        ...
        steps=ttk.Frame(sync); steps.pack(fill=tk.X,padx=6,pady=6)
        self.game_button=ttk.Button(steps,text="1. Locate game folder",command=self._choose_game)
        self.game_button.pack(side=tk.LEFT,padx=(0,4))
        self.save_button=ttk.Button(steps,text="2. Choose save",command=self._choose_save)
        self.save_button.pack(side=tk.LEFT,padx=4)
        self.sync_button=ttk.Button(steps,text="3. Sync game data",command=self._sync_game_data)
        self.sync_button.pack(side=tk.LEFT,padx=4)
        self.sprite_button=ttk.Button(steps,text="4. Extract sprites",command=self._refresh_component_sprites)
        self.sprite_button.pack(side=tk.LEFT,padx=4)
        self.refresh_save_button=ttk.Button(steps,text="5. Refresh save after ritual",command=self._refresh_selected_save)
        self.refresh_save_button.pack(side=tk.LEFT,padx=4)
        ttk.Checkbutton(steps,text="Use save quantities and Morph Analysis bonuses",variable=self.sync_var,command=self._settings_changed).pack(side=tk.LEFT,padx=10)
        self.sync_status=tk.StringVar(value="Manual mode")
        ttk.Label(steps,textvariable=self.sync_status).pack(side=tk.RIGHT,padx=8)

        context=ttk.Labelframe(sync,text="Setup context")
        context.pack(fill=tk.X,padx=6,pady=(0,6))
        self.context_var=tk.StringVar(value="")
        ttk.Label(context,textvariable=self.context_var,justify=tk.LEFT).pack(fill=tk.X,padx=6,pady=4)
        ...
        ttk.Button(sb,text="Clear",command=lambda:self.inventory_search_var.set("")).pack(side=tk.LEFT,padx=5)
        self.hide_unavailable_var=tk.BooleanVar(value=self.settings.hide_unavailable)
        ttk.Checkbutton(sb,text="Hide unavailable",variable=self.hide_unavailable_var,command=self._hide_unavailable_changed).pack(side=tk.LEFT,padx=5)
        self.inventory_search_var.trace_add("write",lambda *_:self._refresh_inventory())
        ...
        self.cancel_button=ttk.Button(act,text="Cancel",command=self._cancel_optimization,state=tk.DISABLED); self.cancel_button.pack(side=tk.LEFT,padx=6); ttk.Button(act,text="Export results",command=self._export_results).pack(side=tk.LEFT); ttk.Button(act,text="Copy selected ritual",command=self._copy_selected_ritual).pack(side=tk.LEFT,padx=6)
    ...
    def _settings_changed(self,*_):
        ...
        hide=bool(self.hide_unavailable_var.get()) if hasattr(self,"hide_unavailable_var") else self.settings.hide_unavailable
        self.settings=AppSettings(p,s,w,bool(self.sync_var.get()),self.settings.game_path,self.settings.save_path,hide); save_settings(self.settings)

    def _detected_game_path(self) -> Path | None:
        game = detect_game_path(self.settings.game_path)
        if game and self.settings.game_path != str(game):
            self.settings.game_path = str(game)
            save_settings(self.settings)
        return game

    def _active_save_path(self) -> Path | None:
        if self.settings.save_path:
            selected = Path(self.settings.save_path)
            if selected.exists():
                return selected
        return None

    def _setup_context_text(self, game: Path | None, save_path: Path | None) -> str:
        game_text = str(game) if game else "Not found yet"
        save_text = str(save_path) if save_path else "Not selected/found yet"
        sprite_count = _confirmed_sprite_count(self.items)
        available = [x for x in self.items if x.enabled and x.quantity > 0]
        units = sum(x.quantity for x in available)
        chips = (
            ("Game OK" if game else "Game missing")
            + " · "
            + ("Save selected" if save_path else "Save missing")
            + f" · Sprites {sprite_count}/{EXPECTED_COMPONENT_SPRITE_COUNT}"
            + f" · Components {len(available)} types/{units} units"
        )
        return (
            f"Status: {chips}\n"
            "Press buttons left to right for first setup. After an in-game ritual, press step 5.\n"
            "Save quantities use only the selected save.\n"
            f"Common game folder: {COMMON_GAME_PATH_HINTS[0]} or {COMMON_GAME_PATH_HINTS[1]}\n"
            rf"Common save path: %USERPROFILE%\AppData\LocalLow\Magnum Scriptum Ltd\Quasimorph\slot_*_session.dat"
            "\n"
            f"Detected game folder: {game_text}\n"
            f"Active save file: {save_text}\n"
            f"Confirmed sprites: {sprite_count} / {EXPECTED_COMPONENT_SPRITE_COUNT}"
        )

    def _refresh_setup_controls(self) -> None:
        if not hasattr(self, "game_button"):
            return
        game = self._detected_game_path()
        save_path = self._active_save_path()
        sprites_ready = _all_confirmed_sprites_ready(self.items)
        self.game_button.configure(state=tk.DISABLED if game else tk.NORMAL)
        self.save_button.configure(state=tk.NORMAL)
        self.sync_button.configure(state=tk.NORMAL if game else tk.DISABLED)
        self.refresh_save_button.configure(state=tk.NORMAL if save_path else tk.DISABLED)
        self.sprite_button.configure(
            state=tk.DISABLED if not game or sprites_ready else tk.NORMAL
        )
        self.context_var.set(self._setup_context_text(game, save_path))

    def _remember_save_snapshot(self, snapshot: SaveSnapshot) -> None:
        self.settings.save_path = str(snapshot.path)
        save_settings(self.settings)

    def _apply_save_snapshot(
        self,
        snapshot: SaveSnapshot,
        status: str = "Save quantities updated",
    ) -> None:
        self.items = _items_with_save_snapshot(self.items, snapshot)
        self.flat_power_var.set(f"{snapshot.power_bonus:g}")
        self.flat_stability_var.set(f"{snapshot.stability_bonus:g}")
        self.sync_var.set(True)
        self._remember_save_snapshot(snapshot)
        self._settings_changed()
        self._save_inventory(silent=True)
        self._refresh_inventory()
        self._refresh_setup_controls()
        self.sync_status.set(f"{status} · {snapshot.path.name}")

    def _hide_unavailable_changed(self):
        self._settings_changed()
        self._refresh_inventory()
    ...
    def _refresh_inventory(self,selected_index=None):
        ...
        for idx in indices:
            ...
            if self.hide_unavailable_var.get() and not available: continue
    ...
    def _choose_game(self):
        ...
        if d:
            self.settings.game_path=d;save_settings(self.settings)
            game=self._detected_game_path()
            if game:self.sync_status.set(f"Game folder ready · {game}")
            else:self.sync_status.set("Selected folder is not a Quasimorph game folder"); messagebox.showerror("Game not found","Select the folder that contains Quasimorph_Data/resources.assets.")
            self._refresh_setup_controls()
    def _choose_save(self):
        ...
        if f:
            self.settings.save_path=f;save_settings(self.settings)
            try:self._apply_save_snapshot(read_save(Path(f)),"Save selected")
            except Exception as exc:messagebox.showerror("Save failed",str(exc));self.sync_status.set("Save selection failed")
    def _refresh_selected_save(self):
        save_path=self._active_save_path()
        if not save_path:
            messagebox.showerror("Save not selected","Choose the current slot_*_session.dat first.")
            return
        try:self._apply_save_snapshot(read_save(save_path),"Selected save refreshed")
        except Exception as exc:messagebox.showerror("Save refresh failed",str(exc));self.sync_status.set("Save refresh failed")
    def _sync_game_data(self):
        try:
            game=self._detected_game_path()
            ...
            for base in db.items:
                new.append(replace(base,sprite_path=previous_sprites.get(base.internal_id,"")))
            if snapshot:new=_items_with_save_snapshot(new,snapshot)
            else:new=[replace(x,quantity=0,enabled=False) for x in new]
            ...
            if snapshot:
                self.flat_power_var.set(f"{snapshot.power_bonus:g}");self.flat_stability_var.set(f"{snapshot.stability_bonus:g}");self._remember_save_snapshot(snapshot)
            ...
            if any(_component_sprite_missing(x) for x in self.items):
                self.sync_status.set(imported_msg + " · press step 4 for sprites")
            else:
    ...
    def _copy_selected_ritual(self):
        sel=self.result_tree.selection()
        if not sel:return
        r=self.results[int(sel[0])]
        self.clipboard_clear()
        self.clipboard_append(r.order_text)
        self.status_var.set("Copied selected ritual order")
