...
def user_data_dir() -> Path:
    ...
    if base: return Path(base) / "QuasimorphRitualOptimizer"
...
def load_settings(path: Path | None = None) -> AppSettings:
    ...
    if not path.exists(): return AppSettings()
    try: payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError): return AppSettings()
    try:
        return AppSettings(
            ...
            sync_save=bool(payload.get("sync_save", False)), game_path=str(payload.get("game_path", "")),
            save_path=str(payload.get("save_path", "")),
            hide_unavailable=bool(payload.get("hide_unavailable", False)),
        )
    except (TypeError, ValueError): return AppSettings()
