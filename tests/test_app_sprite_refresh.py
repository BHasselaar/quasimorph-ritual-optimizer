...
from quasimorph_optimizer.save_sync import SaveSnapshot
...
def test_confirmed_sprite_readiness_requires_all_record_cache_files(monkeypatch, tmp_path: Path):
    cache = tmp_path / "sprites"
    cache.mkdir()
    monkeypatch.setattr(app, "sprite_cache_dir", lambda: cache)
    items = []
    for item_id in USER_CONFIRMED_SPRITE_ALIASES:
        sprite = cache / f"{item_id}.png"
        sprite.write_bytes(b"png")
        items.append(_component(item_id, str(sprite)))

    assert app._confirmed_sprite_count(items) == 74
    assert app._all_confirmed_sprites_ready(items)

    missing = items[:-1] + [_component(items[-1].internal_id, "")]
    assert app._confirmed_sprite_count(missing) == 73
    assert not app._all_confirmed_sprites_ready(missing)


def test_save_snapshot_updates_quantities_without_reenabling_manual_exclusions(tmp_path: Path):
    items = [
        replace(_component("spider_joint"), enabled=False, quantity=3),
        replace(_component("mars_gear"), enabled=True, quantity=2),
        replace(_component("moon_armor_plates"), enabled=False, quantity=0),
    ]
    snapshot = SaveSnapshot(
        tmp_path / "slot_0_session.dat",
        {"spider_joint": 1, "mars_gear": 0, "moon_armor_plates": 4},
        100.0,
        40.0,
        (),
    )

    updated = app._items_with_save_snapshot(items, snapshot)

    assert [(x.internal_id, x.quantity, x.enabled) for x in updated] == [
        ("spider_joint", 1, False),
        ("mars_gear", 0, False),
        ("moon_armor_plates", 4, True),
    ]
